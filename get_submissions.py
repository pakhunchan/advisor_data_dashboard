...
def get_submissions(
    canvas_base_url: str, canvas_bearer_token: str, course_id: int, submitted_since: str
) -> list[dict]:
...
def make_student_id_dict(student_payload: list) -> dict:
    ...
    for student in student_payload:
        try:
            ...
            logging.error(
                f"Failed student dict was: {json.dumps(student, default=str)}"
            )
...
def should_not_skip(submission, student, student_id_dict):
    return (
        ...
        "sis_user_id"
        in student  # keep only if submission has a student number attached
        and student_id_dict.get(
            student["sis_user_id"]
        )  # keep only if student number is in our active student list
    )


# function that takes in an assignment submission and updates canvas_student_participation as necessary
def add_submission(
    canvas_student_participation: dict,
    studentNumber: str,
    submitted_at: str,
    course_id: int,
) -> None:
    ...
    if canvas_student_participation.get(studentNumber):
        if submitted_at < canvas_student_participation.get(studentNumber, {}).get(
            "earliest"
        ):
